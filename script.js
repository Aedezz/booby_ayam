...
// ==============================
// RINGKASAN PEMBAYARAN (Tunai vs QRIS vs Piutang)
// ==============================
function ubahTotalQris(value) {
    state.qris = Number(value) || 0;
    saveState();
    renderRingkasanPembayaran();
}
...
function renderRingkasanPembayaran() {
    ...
    const qrisInput = document.getElementById('inputQrisHariIni');
    if (qrisInput && document.activeElement !== qrisInput) {
        qrisInput.value = totalQris || '';
    }

    const tunaiEl = document.getElementById('totalTunai');
    if (tunaiEl) {
        tunaiEl.innerText = 'Rp ' + totalTunai.toLocaleString('id-ID');
        tunaiEl.className = totalTunai < 0 ? 'font-bold text-red-500' : 'font-bold text-green-600';
    }
}
...
function resetData() {
    ...
    if (typeof resetStockHarian === 'function') {
        resetStockHarian();
    }
}
...
function tampilkanDetailArsip(index) {
    ...
    detailEl.innerHTML = `
        ...
        ${entry.qris ? `<p class="text-[11px] text-gray-400 mb-3">QRIS: Rp ${entry.qris.toLocaleString('id-ID')} &middot; Tunai: Rp ${(entry.total - entry.qris).toLocaleString('id-ID')}</p>` : '<div class="mb-3"></div>'}
...
function copyToWA() {
    ...
    if (totalPiutang > 0) {
        text += `\n*PIUTANG BELUM LUNAS: Rp ${totalPiutang.toLocaleString('id-ID')}*`;
    }
    if (totalQris > 0 || totalPiutang > 0) {
        text += `\n*TUNAI SEHARUSNYA ADA: Rp ${(total - totalQris - totalPiutang).toLocaleString('id-ID')}*`;
    }
